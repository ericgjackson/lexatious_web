...
class Board {
  ...
  public static fromStrings(letterString: string, occString: string, width: number, height: number): Board {
    const rowIndices = Array.from({ length: height }, (_, i) => i);
    const colIndices = Array.from({ length: width }, (_, i) => i);
    return new Board({
      rows: rowIndices.map((row,) =>
        colIndices.map((col,) =>
          new Cell({
            isEmpty: !letterString[row * width + col] || letterString[row * width + col] === EMPTY_CELL,
            tile: letterString[row * width + col] === EMPTY_CELL ? Tile.Null :
              new Tile({
                character: letterString[row * width + col],
                occupier: parseInt(occString[row * width + col], 10),
              }),
            x: col,
            y: row,
          }),
